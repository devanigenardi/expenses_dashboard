...
function App() {
  ...
  const totalAmount =
    activeTrip?.entries.reduce(
      (acc, entry) => acc + parseFloat(entry.amount || 0),
      0
    ) || 0;
  const totalDividedAmount =
    activeTrip?.entries.reduce(
      (acc, entry) => acc + parseFloat(entry.dividedAmount || 0),
      0
    ) || 0;
      ...
      <div className="App-header">
        ...
        <div className="dashboard-main">
          <div className="add-new-trip">
            ...
            />
            <Button className="add-trip" onClick={addNewTrip}>
              Agregar Viaje
            </Button>
          </div>
          ...
          {activeTrip && (
            <>
              <Button className="add-row" onClick={addRowToActiveTrip}>
                Agregar gastos
              </Button>
                  ...
                  <div key={entry.id} className="entry-row">
                    <div className="couple">
                      ...
                      <TextField
                        ...
                        label="Ubicación"
                        variant="outlined"
                        value={entry.location || ""} // Agrega el valor predeterminado para evitar errores si no existe la propiedad
                        onChange={async (e) => {
                    ...
                    <div className="quartets">
                      ...
                      <TextField
                        ...
                        onChange={async (e) => {
                          ...
                          const newDividedAmount = (
                            newAmount / entry.numberOfPeople
                          ).toFixed(2);
                          // Actualizar el estado local
                          setTrips(
                            trips.map((trip) =>
                                    ...
                                    entries: trip.entries.map((item) =>
                                      item.id === entry.id
                                        ? {
                                            ...item,
                                            amount: newAmount,
                                            dividedAmount: newDividedAmount,
                                          }
                                        : item
                                    ),
                                  }
